...
test('hashme sha1 this is some text', function (t) {
	...
	t.equal(
		hashMe('sha1').update('this is some text').digest('hex'),
...
test('hashme sha256 123', function (t) {
	...
	t.equal(
		hashMe('sha256').update('123').digest('hex'),
...
test('hashme sha512 123', function (t) {
	...
	t.equal(
		hashMe('sha512').update('123').digest('hex'),
...
test('hashme md5 123', function (t) {
	...
	t.equal(hashMe('md5').update('123').digest('hex'), '202cb962ac59075b964b07152d234b70', 'to create md5');
